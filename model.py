...
st.markdown("<h1 style='text-align:center;'>EFS TURBO DRIVE</h1>", unsafe_allow_html=True)

#menu = st.sidebar.selectbox(
    #"Menu",
    #["Submit Idea", "PL Assignment", "Feasibility", "Approval", "Dashboard"]
#)

# =========================================================
# SAFE DATA CLEANER (CRITICAL FIX)
# =========================================================
def clean_df(df):
    df = df.copy()
...
if menu == "Submit Idea":
    ...
    # =========================================================
    # GET PL / SPL USERS FROM EXCEL
    # =========================================================
    pl_users = user_df[
        user_df["role"].isin(["pl/spl", "automation pl"])
...
# =========================================================
    ...
    for item in df.to_dict("records"):
        ...
        if item["status"] == "Assigned":
            ...
            st.write(f"👷 Engineer: {item.get('assigned_engineer','-')}")

            manual = st.number_input("Manual Effort", key=f"m_{item['id']}")
            fte = st.number_input("FTE", key=f"f_{item['id']}")
            eng = st.number_input("Automation Effort", key=f"e_{item['id']}")
            freq = st.selectbox(
                "Frequency",
...
# =========================================================
    ...
    pl_list = sorted(df["pl_name"].dropna().unique().tolist())
    pl_user = st.selectbox("Select PL / SPL Name", [""] + pl_list)
    ...
    for item in data:
        ...
        if decision == "NO-GO":
            ...
            reason = st.selectbox(
                "Reason",
                ["Technical Rejection","Business Rejection"],
                key=f"r_{item['id']}"
            )
